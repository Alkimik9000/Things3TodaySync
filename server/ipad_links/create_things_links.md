...
`server/ipad_links/create_things_links.py` converts rows in `processed_tasks.csv` into URLs that use the `things:///add` scheme. Each URL opens Things on iPadOS or macOS and pre‑populates a new task.
...
```
things:///add?title=Buy%20milk&notes=Check%20expiration&deadline=2025-06-16
```
